...
def gatherAllCSVs(apolloType, xplrPol, imbal, numPol, depth, trainSize):
	...
	# This makes the files too huge, so we ommit them
...
def main():
	...
	parser.add_argument('--apolloType', type=str, required=True, help='the type of lulesh execution')
	parser.add_argument('--xplrPol', type=str, required=False)
	parser.add_argument('--imbal', type=str, required=True)
	parser.add_argument('--numPol', type=str, required=True)
	parser.add_argument('--depth', type=str, required=True)
	parser.add_argument('--trainSize', type=str, required=True)

	args = parser.parse_args()

	apolloType = args.apolloType
	xplrPol = args.xplrPol
	imbal = args.imbal
	numPol = args.numPol
	depth = args.depth
